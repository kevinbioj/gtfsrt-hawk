...
async function loadStops(resourcePath: string) {
	const stops = await readFile(join(resourcePath, "stops.txt"))
		.then((buffer) => buffer.toString())
		.then(parseCsv);
...
async function loadTrips(
	...
	const trips = await readFile(join(resourcePath, "trips.txt"))
		.then((buffer) => buffer.toString())
		.then(parseCsv);
	const routes = await readFile(join(resourcePath, "routes.txt"))
		.then((buffer) => buffer.toString())
		.then(parseCsv);

	const stopTimes = groupBy(
		await readFile(join(resourcePath, "stop_times.txt"))
			.then((buffer) => buffer.toString())
			.then((value) =>
				parseCsv<{
					trip_id: string;
	...
	return trips.map(
		(trip) =>
			({
				...
				route: {
					id: trip.route_id,
					name: routes.find((r) => r.route_id === trip.route_id)!
						.route_short_name,
				},
