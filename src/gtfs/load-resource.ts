...
import { getOperatingTrips } from "../utils/get-operating-trips.js";
...
export async function useGtfsResource(resourceUrl: string) {
	...
	const resource = {
		...
		operatingTrips: getOperatingTrips(initialResource.resource),
	...
	if (operatingTripsJob === undefined) {
		operatingTripsJob = new Cron("0 3 * * *", () => {
			resource.operatingTrips = getOperatingTrips(resource.gtfs);
		});
	}
